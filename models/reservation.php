...
class Reservation
{
    ...
    public function get()
    {
        $id = $_SESSION["idUser"];
        $arrayResult = array();
        $result = $this->db->consulta("SELECT reservations.id, resources.name, users.realname, users.id AS userid, timeslots.starTime, reservations.date, reservations.remarks
             FROM reservations, resources, users, timeslots
                ...
                ORDER BY reservations.date, timeslots.starTime");
    ...
    public function getByDate($date, $timeslot, $idrecurso) {
        $result = $this->db->consulta("SELECT reservations.id
             FROM reservations
                WHERE reservations.date = '$date'
                AND reservations.idTimeslot = '$timeslot'
                AND reservations.idResource = '$idrecurso'");
        return $result;
    }
